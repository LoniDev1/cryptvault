...
impl KdfParams {
    pub fn strong(salt: Vec<u8>) -> Self {
        Self { m_cost_kib: 256 * 1024, t_cost: 8, p_cost: 1, salt }
    }
    pub fn paranoid(salt: Vec<u8>) -> Self {
        Self { m_cost_kib: 1024 * 1024, t_cost: 12, p_cost: 1, salt }
    }
    pub fn fast(salt: Vec<u8>) -> Self {
        Self { m_cost_kib: 64 * 1024, t_cost: 3, p_cost: 1, salt }
    }
}

pub fn derive_master_key(password: &[u8], params: &KdfParams) -> Result<[u8; MASTER_KEY_LEN]> {
    let p = Params::new(params.m_cost_kib, params.t_cost, params.p_cost, Some(MASTER_KEY_LEN))
