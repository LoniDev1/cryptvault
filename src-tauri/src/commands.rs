...
pub async fn unmount_volume(state: State<'_, AppState>, id: String) -> Result<()> {
    let mut m = state
        ...
        .lock()
        .remove(&id)
        .ok_or_else(|| AppError::NotFound("mount".into()))?;
...
pub async fn encrypt_path(path: String, password: String, kdf_mode: KdfMode) -> Result<String> {
    ...
    let out = tokio::task::spawn_blocking(move || {
        files_mod::encrypt_file(&p, password.as_bytes(), kdf_mode)
    })
    .await
    .map_err(|e| AppError::Other(e.to_string()))??;
...
pub async fn decrypt_path(path: String, password: String, out_dir: String) -> Result<String> {
    ...
    let out =
        tokio::task::spawn_blocking(move || files_mod::decrypt_file(&p, password.as_bytes(), &o))
            .await
            .map_err(|e| AppError::Other(e.to_string()))??;
    Ok(out.to_string_lossy().to_string())
}
...
pub fn password_strength(password: String) -> PasswordReport {
    ...
    if len < 16 {
        s.push("use at least 16 characters");
    }
    PasswordReport {
        score,
...
pub fn generate_passphrase(words: u8) -> String {
    const WORDS: &[&str] = &[
        "alpha", "bravo", "cobalt", "delta", "ember", "forge", "glacier", "harbor", "ivory",
        "jade", "krypton", "lumen", "mosaic", "nebula", "onyx", "pulse", "quartz", "raven",
        "silver", "tango", "umbra", "vector", "willow", "xenon", "yarrow", "zephyr", "atlas",
        "beacon", "cipher", "drift", "echo", "flint", "gravity", "helix", "indigo", "junction",
        "kernel", "lattice", "monarch", "nova", "orbit", "prism", "quasar", "ridge", "saber",
        "talon", "union", "verve", "whisper", "azure",
    ];
    let mut rng = rand::thread_rng();
    let n = words.clamp(4, 12) as usize;
    let mut chosen: Vec<String> = (0..n)
